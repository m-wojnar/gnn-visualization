...
from torch_geometric.nn import GCNConv, CGConv, GATConv
...
if __name__ == '__main__':
    ...
    model = VisAttentionGNN(input_dim=X.shape[1], hidden_dim=args.hidden_dim, num_layers=args.num_layers).to(device)
    model, H = train(model, graph, args.epochs, args.lr)
    H = np.stack(H)

    a = utils.EmbeddingsScatterAnimation(H, graph.y, xlim=(0, 1), ylim=(0, 1))
